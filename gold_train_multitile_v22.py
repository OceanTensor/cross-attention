...
# v22 채널 ablation: ch15(Turbidity) 제외하고 재학습
# 이유: XAI(gradient saliency) 분석 중 발견 — 완도 4일/목포 7일 
# 극희소 관측(365일 중 고유값 4~7개)로 인한 계단식 아티팩트가 
# saliency 1위로 잡히는 착시를 만듦(값 자체는 정상범위라 
# DIN_DIP_ratio류 계산버그와는 다른 종류의 문제). 
# v10/v11에서도 이미 탁도 단독/포함 시 성능 하락을 확인한 전례가 있어
# 이번 ablation으로 최종 확인한다.
EMPTY_CHANNELS = {"ch15"}
