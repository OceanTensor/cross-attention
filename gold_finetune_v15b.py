"""
gold_finetune_v15b.py — v15 + pos_weight 보정 (확률 캘리브레이션 개선판) — v13 사전학습 백본 → 32채널 완도 데이터 전체 파인튜닝 (v15)
════════════════════════════════════════════════════════════════════════════
...
def compute_pos_weight(dataset):
    """train 데이터의 실제 양성(경계 이상) 비율로 pos_weight 계산.
    sigmoid 출력이 극단적으로 낮은 구간에 몰리는 문제(v15 1차 결과)를
    완화하기 위해, BCEWithLogitsLoss의 pos_weight로 양성 신호를 증폭."""
    pos, total = 0, 0
    for s in dataset.samples:
        sev = dataset.frame_severity.get(s["target_frame"], 0)
        total += 1
        if SEVERITY_TO_ADI[sev] >= WARN_THRESH:
            pos += 1
    neg = total - pos
    pw = neg / max(pos, 1)
    print(f"  pos_weight 계산: 양성 {pos}/{total} ({pos/total*100:.1f}%) -> pos_weight={pw:.2f}")
    return pw


def warn_only_loss(outputs, adi_target, warn_thresh=WARN_THRESH, pos_weight=None):
    """adi_head/severe_head는 학습에서 제외 (프록시 타깃이라 회귀 오염 위험,
    심각=3 실측 없어 severe 학습 근거 없음). warn_head만 BCE로 파인튜닝.
    pos_weight로 양성 클래스 손실을 증폭해 sigmoid bias가 우리 데이터
    분포(양성 8.7%)로 더 빠르게 이동하도록 유도 (확률 캘리브레이션 개선)."""
    import torch.nn.functional as F
    import torch
    warn_logit = outputs["warn_logit"]              # (B, Hf, Wf)
    pix_max = adi_target.amax(dim=1)                 # (B, ph, pw) — 프록시라 전부 유효
    ph, pw_ = pix_max.shape[1], pix_max.shape[2]
    ...
    pw_tensor = torch.tensor(pos_weight, device=warn_logit.device) if pos_weight else None
    bce_warn = F.binary_cross_entropy_with_logits(warn_logit, warn_tgt, pos_weight=pw_tensor)
...
def evaluate(model, loader, device):
    ...
    f1_at_05 = f1_score(y_true, y_pred_05, average="macro", zero_division=0)
    cm = confusion_matrix(y_true, y_pred_05)
    print(f"    [진단] prob 분포: min={y_prob.min():.4f} max={y_prob.max():.4f} "
          f"평균={y_prob.mean():.4f}")
    return {"auc": auc, "pr_auc": ap, "f1_macro": f1_at_05, "confusion_matrix": cm.tolist(),
            "prob_mean": float(y_prob.mean()), "prob_max": float(y_prob.max())}
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for x, adi_target, sev in dl_train:
            ...
            out = model(x)
            loss, _ = warn_only_loss(out, adi_target, pos_weight=pos_weight)
        ...
        print(f"  Epoch {epoch:3d}/{args.epochs} | loss={ep_loss:.4f} | "
              f"val_AUC={ev['auc']:.4f} val_F1@0.5={ev['f1_macro']:.4f} "
              f"prob_mean={ev['prob_mean']:.4f} | {elapsed:.0f}s")
        history.append({"epoch": epoch, "train_loss": ep_loss, **{k: v for k, v in ev.items() if k != "confusion_matrix"}})

        if ev["f1_macro"] > best_auc:  # 변수명 유지, 실제 기준은 F1@0.5
            best_auc = ev["f1_macro"]; best_epoch = epoch
