...
# v11 전용: cube_id=36(=v7 30채널 + Turbidity만, NIR_daily_change 없음)에 맞춤
# v10(32채널, Turbidity+NIR변화량 동시추가)이 v7보다 성능 하락한 원인을
# 분리 진단하기 위해 Turbidity 단독 효과만 확인하는 실험(v11).
# ch29(NIR_daily_change)는 이 큐브에 아직 반영 안 됨 → 다시 빈 채널로 취급
EMPTY_CHANNELS = {"ch29"}
VALID_CHANNELS = [ch for ch in CHANNEL_ORDER if ch not in EMPTY_CHANNELS]  # 31개
